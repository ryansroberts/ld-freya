...
module Tools =
  ...
  let step f (PipelineStep(m,xp)) = async {
    let xr = xp
               |> List.map either
  ...
  let private contentS m _ = async {
    return pipeline.succeed [info "Apply content tool" (fileLocation m.Target.Path)]
            [owl.individual m.Target.Id [m.Represents] [ dataProperty !"cnt:chars" (m.Target.Content ^^ xsd.string) ]]}

  let content = step contentS

  type YNode = Freya.YamlParser.Node

  let yamlMetadataS m _ = async {
    ...
    let yamlToStatements y =
      try
        pipeline.succeed [info "Extracting metadata from yaml codeblock" (fileLocation m.Target.Path) ]
                [rdf.resource m.Target.Id ( translate ( parse y ) )]
      with e ->
        pipeline.succeed [warn (sprintf "Failed to parse yaml: %s" e.Message) (fileLocation m.Target.Path)] []

    let md = Markdown.Parse m.Target.Content
    match md.Paragraphs with
    | CodeBlock(yaml, _, _) :: _ -> return yamlToStatements yaml
    | _ ->
        return pipeline.succeed [ warn "No metadata block at start of file" (fileLocation m.Target.Path) ] []
    }
  ...
  let convertMarkdownS t m xr  =
    match xr with
      | r::xr -> async {
        ...
        return pipeline.succeed [prov] []}
      | [] ->  async { return pipeline.fail []}

  let convertMarkdown t = step (convertMarkdownS (t,{Output=toPath ".";WorkingDir= toPath "."}))
