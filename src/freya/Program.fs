...
type Model.Uri with
  static member fromVDS (n:INode) = match n with | :? IUriNode as n -> Model.Uri (string n) | _ -> failwith "Not a uri node"
...
let loadCompilation (g:Graph) =
  ...
  {
      Id =  resultset "select ?id where {?id a compilation:Compilation}"
            |> Seq.exactlyOne
            |> single :?> IUriNode
            |> Uri.fromVDS
      Targets = resultset """
