...
[<Fact>]
let ``Compile provenance to content ontology``() =
  let matchingTarget =
...
"""
  ...
  let g = graph.loadFormat graph.parse.ttl (graph.fromString prov)
  let prov = loadProvenance g
  test <@  prov =
    {
